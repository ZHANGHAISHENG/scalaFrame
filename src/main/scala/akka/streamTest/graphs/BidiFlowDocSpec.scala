...
object BidiFlowDocSpec {
  ...
  def fromBytes(bytes: ByteString): Message = {
    ...
    it.getByte match {
      case 1     ⇒ println("fromBytes：ping"); Ping(it.getInt)
      case 2     ⇒ println("fromBytes：pong"); Pong(it.getInt)
...
class BidiFlowDocSpec extends AkkaSpec {
  ...
  "A BidiFlow" must {
    ...
    "compose" in {
      ...
      val pingpong = Flow[Message].collect { case Ping(id) ⇒ println("flow: ping -> pong");Pong(id)}
      val flow = stack.atop(stack.reversed).join(pingpong)
      val result = Source((0 to 1).map(Ping)).via(flow).limit(20).runWith(Sink.seq)
    ...
    "work when chopped up" in {
      val stack = codec.atop(framing)
      val flow = stack.atop(chopUp).atop(stack.reversed).join(Flow[Message].map { case Ping(id) ⇒println("flow: ping -> pong"); Pong(id) })
      val f = Source((0 to 1).map(Ping)).via(flow).limit(20).runWith(Sink.seq)
    ...
    "work when accumulated" in {
      val stack = codec.atop(framing)
      val flow = stack.atop(accumulate).atop(stack.reversed).join(Flow[Message].map { case Ping(id) ⇒println("flow: ping -> pong"); Pong(id) })
      val f = Source((0 to 1).map(Ping)).via(flow).limit(20).runWith(Sink.seq)
      val r = Await.result(f, 1.second)
      println(r)
    }
    ...
    /**
      *
      * 输出：
      emit:ByteString(1, 0, 0, 0, 0)
      fromBytes：ping
      flow: ping -> pong
      emit:ByteString(2, 0, 0, 0, 0)
      fromBytes：pong
      emit:ByteString(1, 1, 0, 0, 0)
      fromBytes：ping
      flow: ping -> pong
      emit:ByteString(2, 1, 0, 0, 0)
      fromBytes：pong
      Vector(Pong(0), Pong(1))
      */

...
class Integration  extends AkkaSpec {
  ...
  class AddressSystem {
    def lookupEmail(handle: String): Future[Option[String]] = Future.successful(Some(handle + "@somewhere.com"))
    def lookupPhoneNumber(handle: String): Future[Option[String]] = Future.successful(Some(handle.hashCode.toString))
  }

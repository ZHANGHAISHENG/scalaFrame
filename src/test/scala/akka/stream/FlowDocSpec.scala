...
class FlowDocSpec extends AkkaSpec with CompileOnlySpec {
  ...
  "various ways of transforming materialized values" in {
    ...
    val throttler: Flow[Int, Int, Cancellable] = Flow.fromGraph(GraphDSL.create(Source.tick(1.second, 1.second, "test")) { implicit builder ⇒tickSource ⇒
    ...
    r1.run().completeWith(Future { Option(1) }).future.onComplete {
